...
export async function uploadFile(file, type) {
  if (!file) {
    console.error("🚨 Error: No file provided to uploadFile.");
    return null;
  }
  ...
  // Allowed extensions (Should match Appwrite settings)
  const allowedExtensions = ["jpg", "jpeg", "png", "gif", "mp4", "mov", "avi", "mkv", "pdf", "docx", "xlsx"];
  if (!allowedExtensions.includes(fileExtension)) {
    console.error(`🚨 Error: File extension .${fileExtension} is not allowed.`);
    return null;
  }
  ...
  try {
    ...
    const fileObj = new File([fileBlob], file.fileName || `upload.${fileExtension}`, {
      type: file.mimeType || file.type,
    });

    console.log("✅ Final File Object:", fileObj);
